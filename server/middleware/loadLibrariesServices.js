...
export async function loadLibrariesServicesData(req, res, next) {
  try {
    const url = 'https://data.princegeorgescountymd.gov/Education/Libraries/7k64-tdwr'; // remote URL! you can test it in your browser
    const data = await fetch(url); // We're using a library that mimics a browser 'fetch' for simplicity
    const json = await data.json(); // the data isn't json until we access it using dot notation
    const reply = json.filter((item) => Boolean(item.geocoded_column_1)).filter((item) => Boolean(item.name));
    console.log('Results in LibrariesServices middleware', json.length); // let's check that something's there before we return it
